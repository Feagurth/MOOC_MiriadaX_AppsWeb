...
$(function () {
    ...
    // Asignamos una función al evento click y al evento tap sobre el botón de compactar
    $("#compactar").on("click tap", function () {

        // Comprobamos que tenemos al menos dos marcadores, puesto que compactar 
        // una ruta con dos marcadores o menos no tiene lógica
        if (map.markers.length > 2)
        {
            // Si tenemos más de dos marcadores creamos una ventana para pedir confirmación al usuario
            $.confirm({
                // Asignamos título a la ventana
                title: '¿Desea compactar la ruta actual?',
                // Permitimos el uso del teclado para interactuar con la ventana
                keyboardEnabled: true,
                // Asignamos el botón intro del teclado para la confirmación
                confirmKeys: [13],
                ...
                cancelKeys: [27],
                // Definimos el texto a mostrar
                content: 'Se eliminarán los marcadores intermedios y se calculará la ruta entre el marcador inicial y el final',
                // Definimos el tema de la ventana
                theme: 'white',
                // Creamos la función que se ejecutará cuando se pulse el botón de aceptar
                confirm: function () {

                    // Almacenamos los valores de latitud y longitud del primer y ultimo marcador del mapa
                    var marcadorInicio = [map.markers[0].getPosition().lat(), map.markers[0].getPosition().lng()];
                    var marcadorFin = [map.markers[map.markers.length - 1].getPosition().lat(), map.markers[map.markers.length - 1].getPosition().lng()];

                    // Creamos un nuevo mapa
                    map = new GMaps({
                        // El mapa estará anclado al elemento con id map
                        el: '#map',
                        // Especificamos la longitud  y latitud donde se centrará el mapa, en este caso el marcador inicial
                        lat: marcadorInicio[0],
                        lng: marcadorInicio[1],
                        // Definimos las funciones a ejecutarse al hacer click o tap
                        click: enlazarMarcador,
                        tap: enlazarMarcador
                    });

                    // Añadimos los marcadores inicial y final al mapa
                    map.addMarker({lat: marcadorInicio[0], lng: marcadorInicio[1]});
                    map.addMarker({lat: marcadorFin[0], lng: marcadorFin[1]});

                    // Dibujamos la ruta entre los dos marcadores
                    dibujarRuta(marcadorInicio, marcadorFin);


                    // Almacenamos el array de posiciones en localStorage despues de transformarlo en una cadena JSON
                    localStorage.posiciones = JSON.stringify(posicionesAArray(map.markers));
        ...
        else
        {
            $.alert({
                title: '¡Atención!',
                content: 'Debe tener al menos tres marcadores para poder compactar una ruta'});
...
/**
 * Función que nos permite convertir los marcadores de un mapa en un array
 * @param {map.markers} marcadores Los marcadores del mapa
 * @returns {Array|posiciones} Devuelve un array de arrays de latitud y longitud
 */
function posicionesAArray(marcadores)
{
    // Definimos un array donde almacenaremos las posiciones de los marcadores
    posiciones = new Array();

    // Iteramos por todos los marcadores que se han pasado como parámetro
    marcadores.forEach(function (elem) {
